...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['family-owner', 'family-member', 'admin', 'viewer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<FamilyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/families` : null),
    () => getFamilies({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Family',
      path: isTenantUser ? `/families/view/${data?.[0]?.id}` : '/families',
      entity: 'family',
      service: AccessServiceEnum.PROJECT,
      icon: FiUsers,
    },
    { name: 'Memory', path: '/memories', entity: 'memory', service: AccessServiceEnum.PROJECT, icon: FiBookOpen },
